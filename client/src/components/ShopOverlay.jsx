import React, { useEffect, useState } from "react";

const RARITY_STYLE = {
  R: {
    border: "hover:border-sky-500/50",
    badge: "bg-sky-500/20 text-sky-300",
    btn: "bg-sky-600 hover:bg-sky-500",
  },
  SR: {
    border: "hover:border-purple-500/50",
    badge: "bg-purple-500/20 text-purple-300",
    btn: "bg-purple-600 hover:bg-purple-500",
  },
  SSR: {
    border: "hover:border-yellow-500/50",
    badge: "bg-yellow-500/20 text-yellow-300",
    btn: "bg-yellow-500 hover:bg-yellow-400 text-black",
  },
};

const ITEM_ICON = {
  r_blue: "🔵",
  r_pink: "🌸",
  sr_dark: "🌙",
  sr_gold: "👑",
  ssr_matrix: "👾",
  shop_aurora: "🌌",
  shop_crown: "💎",
};

function ShopOverlay({
  isOpen,
  onClose,
  apiUrl,
  authFetch,
  userData,
  onRedeemSuccess,
  onBuyTicket,
}) {
  const [activeTab, setActiveTab] = useState("cosmetics"); // 'cosmetics' | 'boosters'
  const [shards, setShards] = useState(0);
  const [items, setItems] = useState([]);
  const [ticketPrice, setTicketPrice] = useState(150);
  const [shieldPrice, setShieldPrice] = useState(200);
  const [loading, setLoading] = useState(true);
  const [redeeming, setRedeeming] = useState(null);
  const [buyingShield, setBuyingShield] = useState(false);

  const shieldOwned = userData?.streak_shield || 0;

  useEffect(() => {
    if (!isOpen) return;
    setLoading(true);
    authFetch(`${apiUrl}/api/shop/items`)
      .then((res) => res.json())
      .then((data) => {
        setShards(data.shards || 0);
        setItems(data.items || []);
        if (data.ticketPrice) setTicketPrice(data.ticketPrice);
        if (data.shieldPrice) setShieldPrice(data.shieldPrice);
      })
      .catch((err) => console.error("Shop fetch error:", err))
      .finally(() => setLoading(false));
  }, [isOpen, apiUrl]);

  const handleRedeem = (item) => {
    setRedeeming(item.id);
    authFetch(`${apiUrl}/api/shop/redeem`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ itemId: item.id }),
    })
      .then((res) => res.json())
      .then((data) => {
        if (data.error) {
          alert(data.error);
          return;
        }
        setItems((prev) => prev.filter((i) => i.id !== item.id));
        setShards((prev) => prev - item.price);
        if (onRedeemSuccess) onRedeemSuccess(data.user);
      })
      .catch((err) => console.error("Redeem error:", err))
      .finally(() => setRedeeming(null));
  };

  const handleBuyShield = () => {
    if (shards < shieldPrice || buyingShield) return;
    setBuyingShield(true);
    authFetch(`${apiUrl}/api/shop/buy-shield`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    })
      .then((res) => res.json())
      .then((data) => {
        if (data.error) {
          alert(data.error);
          return;
        }
        setShards((prev) => prev - shieldPrice);
        if (onRedeemSuccess) onRedeemSuccess(data.user);
      })
      .catch((err) => console.error("Buy shield error:", err))
      .finally(() => setBuyingShield(false));
  };

  const handleBuyTicket = () => {
    if (shards < ticketPrice) return;
    if (onBuyTicket) onBuyTicket();
  };

  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 animate-fade-in">
      <div className="bg-slate-900 border border-slate-700 w-full max-w-2xl rounded-2xl p-6 text-white shadow-2xl relative flex flex-col max-h-[85vh]">
        {/* Header */}
        <div className="flex justify-between items-center border-b border-slate-800 pb-4 mb-4">
          <div className="flex items-center gap-3">
            <span className="text-3xl">🛍️</span>
            <div>
              <h2 className="text-xl font-black text-emerald-400 tracking-wide">
                SHARDS EXCHANGE
              </h2>
              <p className="text-xs text-gray-400">
                Tukar Shards hasil duplicate gacha
              </p>
            </div>
          </div>

          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1.5 bg-slate-800/60 border border-slate-700 rounded-full px-3 py-1.5">
              <span className="text-xs">💠</span>
              <span className="font-black text-xs text-cyan-300">{shards}</span>
              {shieldOwned > 0 && (
                <span className="flex items-center gap-0.5 pl-1.5 ml-0.5 border-l border-slate-700 text-cyan-400 text-[10px] font-bold">
                  🛡️ {shieldOwned}
                </span>
              )}
            </div>

            <button
              type="button"
              onClick={onClose}
              className="text-gray-400 hover:text-white font-bold text-xl p-2 rounded-lg hover:bg-slate-800"
            >
              ✕
            </button>
          </div>
        </div>

        {/* Tab Switcher */}
        <div className="flex bg-slate-950/80 p-1.5 rounded-xl border border-slate-800 text-xs font-black mb-4">
          <button
            onClick={() => setActiveTab("cosmetics")}
            className={`flex-1 py-2.5 rounded-lg transition-all ${
              activeTab === "cosmetics"
                ? "bg-emerald-500 text-slate-950 shadow-md"
                : "text-slate-400 hover:text-slate-200"
            }`}
          >
            Cosmetics
          </button>
          <button
            onClick={() => setActiveTab("boosters")}
            className={`flex-1 py-2.5 rounded-lg transition-all ${
              activeTab === "boosters"
                ? "bg-emerald-500 text-slate-950 shadow-md"
                : "text-slate-400 hover:text-slate-200"
            }`}
          >
            Boosters
          </button>
        </div>

        {/* TAB: COSMETICS */}
        {activeTab === "cosmetics" && (
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-2.5 overflow-y-auto pr-1 flex-1">
            {loading ? (
              <div className="col-span-full text-center text-sm text-gray-500 py-8">
                Loading...
              </div>
            ) : items.length === 0 ? (
              <div className="col-span-full text-center text-sm text-gray-500 py-8">
                🎉 Kamu udah punya semua kosmetik!
              </div>
            ) : (
              items.map((item) => {
                const style = RARITY_STYLE[item.rarity];
                const canAfford = shards >= item.price;
                return (
                  <div
                    key={item.id}
                    className={`bg-slate-800/80 p-2.5 rounded-lg border border-slate-700 flex flex-col items-center text-center justify-between transition-all ${style.border}`}
                  >
                    <span
                      className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${style.badge}`}
                    >
                      {item.rarity}
                    </span>
                    <div className="text-2xl my-1.5">{ITEM_ICON[item.id]}</div>
                    <h4 className="font-bold text-[11px] leading-tight text-slate-100 line-clamp-2 min-h-[28px] flex items-center">
                      {item.name}
                    </h4>
                    {item.shopOnly && (
                      <span className="text-[8px] text-fuchsia-400 font-bold mt-0.5">
                        ✦ EXCLUSIVE
                      </span>
                    )}
                    <button
                      onClick={() => handleRedeem(item)}
                      disabled={!canAfford || redeeming === item.id}
                      className={`mt-2 px-2 py-1.5 text-[10px] font-black rounded-md w-full transition-all disabled:opacity-40 disabled:cursor-not-allowed ${style.btn}`}
                    >
                      {redeeming === item.id ? "..." : `💠${item.price}`}
                    </button>
                  </div>
                );
              })
            )}
          </div>
        )}

        {/* TAB: BOOSTERS */}
        {activeTab === "boosters" && (
          <div className="grid grid-cols-2 gap-3 overflow-y-auto pr-1 flex-1 content-start">
            {/* Gacha Ticket */}
            <div className="bg-gradient-to-br from-slate-800 to-slate-900 border border-indigo-500/40 rounded-lg p-3 flex flex-col items-center text-center">
              <div className="text-3xl my-1">🎟️</div>
              <h4 className="font-black text-xs text-indigo-300">
                Gacha Ticket
              </h4>
              <p className="text-[10px] text-gray-400 mt-1 mb-2 leading-snug">
                1x pull gratis, gak potong Gems
              </p>
              <button
                onClick={handleBuyTicket}
                disabled={shards < ticketPrice}
                className="w-full mt-auto py-1.5 font-black rounded-md active:scale-95 transition-all text-[10px] shadow-lg bg-indigo-600 text-white hover:bg-indigo-500 disabled:opacity-40 disabled:cursor-not-allowed"
              >
                💠 {ticketPrice}
              </button>
            </div>

            {/* Streak Shield */}
            <div className="bg-gradient-to-br from-slate-800 to-slate-900 border border-cyan-500/40 rounded-lg p-3 flex flex-col items-center text-center">
              <div className="text-3xl my-1">🛡️</div>
              <h4 className="font-black text-xs text-cyan-300">
                Streak Shield
              </h4>
              <p className="text-[10px] text-gray-400 mt-1 mb-1 leading-snug">
                Streak gak reset walau kelewat 1 hari
              </p>
              {shieldOwned > 0 && (
                <span className="text-[9px] text-cyan-400 font-bold mb-1">
                  Dimiliki: {shieldOwned}
                </span>
              )}
              <button
                onClick={handleBuyShield}
                disabled={shards < shieldPrice || buyingShield}
                className="w-full mt-auto py-1.5 font-black rounded-md active:scale-95 transition-all text-[10px] shadow-lg bg-cyan-600 text-white hover:bg-cyan-500 disabled:opacity-40 disabled:cursor-not-allowed"
              >
                {buyingShield ? "..." : `💠 ${shieldPrice}`}
              </button>
            </div>
          </div>
        )}
      </div>
    </div>
  );
}

export default ShopOverlay;
