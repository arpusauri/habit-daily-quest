import React from "react";

// Daftar lengkap semua item yang ada di game
const MASTER_ITEMS = [
  {
    id: "r_blue",
    name: "Cyan Border",
    rarity: "R",
    desc: "Border warna cyan yang elegan.",
  },
  {
    id: "r_pink",
    name: "Pink Text Font",
    rarity: "R",
    desc: "Font dengan warna pink ceria.",
  },
  {
    id: "sr_dark",
    name: "Obsidian Dark Theme",
    rarity: "SR",
    desc: "Tema gelap yang misterius.",
  },
  {
    id: "sr_gold",
    name: "Golden Name Tag",
    rarity: "SR",
    desc: "Nametag emas berkilau sultan.",
  },
  {
    id: "ssr_matrix",
    name: "Animated Cyberpunk Matrix",
    rarity: "SSR",
    desc: "Animasi hujan kode matrix.",
  },
];

const ItemIndex = ({ userData, onClose }) => {
  // Helper untuk mengecek apakah item ada di inventory user
  const checkIsOwned = (itemId) => {
    if (!userData?.inventory) return false;
    // Cek apakah array inventory berisi string (ID) atau object ({id: ...})
    return userData.inventory.some(
      (item) => item === itemId || item.id === itemId,
    );
  };

  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-slate-900 border-2 border-slate-600 rounded-2xl w-full max-w-2xl max-h-[80vh] flex flex-col overflow-hidden shadow-2xl">
        {/* Header */}
        <div className="p-4 border-b border-slate-700 flex justify-between items-center bg-slate-800">
          <div>
            <h2 className="text-xl font-bold text-white tracking-wider">
              📚 Item Collection
            </h2>
            <p className="text-xs text-slate-400 mt-1">
              {MASTER_ITEMS.filter((i) => checkIsOwned(i.id)).length} /{" "}
              {MASTER_ITEMS.length} Unlocked
            </p>
          </div>
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-white text-2xl font-bold p-2"
          >
            ×
          </button>
        </div>

        {/* List Items */}
        <div className="p-4 overflow-y-auto grid grid-cols-1 sm:grid-cols-2 gap-4">
          {MASTER_ITEMS.map((item) => {
            const isOwned = checkIsOwned(item.id);

            // Styling berdasarkan rarity (hanya aktif kalau owned)
            const rarityColors = {
              SSR: isOwned
                ? "text-yellow-400 border-yellow-500/50 bg-yellow-500/10"
                : "",
              SR: isOwned
                ? "text-purple-400 border-purple-500/50 bg-purple-500/10"
                : "",
              R: isOwned
                ? "text-blue-400 border-blue-500/50 bg-blue-500/10"
                : "",
            };

            return (
              <div
                key={item.id}
                className={`p-4 rounded-xl border-2 flex flex-col gap-2 transition-all ${
                  isOwned
                    ? rarityColors[item.rarity]
                    : "bg-slate-800/50 border-slate-700 grayscale opacity-60"
                }`}
              >
                <div className="flex justify-between items-start">
                  <span
                    className={`text-xs font-black px-2 py-0.5 rounded-sm ${
                      isOwned
                        ? "bg-slate-900 shadow-sm"
                        : "bg-slate-700 text-slate-400"
                    }`}
                  >
                    {item.rarity}
                  </span>
                  {!isOwned && (
                    <span className="text-slate-500 text-sm">🔒 Locked</span>
                  )}
                  {isOwned && (
                    <span className="text-green-400 text-sm">✔️ Owned</span>
                  )}
                </div>

                <h3
                  className={`font-bold mt-1 ${isOwned ? "" : "text-slate-400"}`}
                >
                  {isOwned ? item.name : "???"}
                </h3>

                <p className="text-xs text-slate-500 line-clamp-2">
                  {isOwned
                    ? item.desc
                    : "Terus kumpulkan gem dan gacha untuk membuka item rahasia ini."}
                </p>
              </div>
            );
          })}
        </div>
      </div>
    </div>
  );
};

export default ItemIndex;
